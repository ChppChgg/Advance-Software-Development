...
def populate_films(self, films):
    ...
    try:
        cursor.executemany(
            ...
            actors = "Jack Black, Jason Mamoa, Sebastian Hansen",
            genre = "Comedy",
            rating = "PG",
            duration = 101
        )
        conn.commit()
        self.close()
        return True, f"{cursor.rowcount} films added successfully."
    except Exception as e:
        conn.rollback()
        self.close()
        return False, f"Error adding films: {e}"
...
films_to_add = [
    ...
    ("DROP", "First dates are nerve-wracking enough. Going on a first date while an unnmaed unseen troll pings you person memes that escalate from annoying to homicidal?", "Meghann Fahy, Brandon Sklenar, Violett Beane, Jacob Robinson, Ed Weeks, Jeffery Self", "Thriller","15", 95)
    ("Death of a unicorn","Father-Daughter duo, Riley and Elliot, hit a unicorn with their car and bring it to the wilderness retreat of a mega-waealthy pharamaceutical CEO.", "Paul Rudd, Jenna Ortega, Téa Leoni, Will Poulter, Richard E Grant", "Comedy / Horror ", "15", 107)
    ("Until Dawn", "One year after her sister Melanie mysteriously disappeared, Clover and her freidns head into the remote valley where she vanished in search of answers.", "Ella Rubin, Michael Cimino, Odessa Azion, Ji-young Yoo, Belmont Cameli, Maia Mitchell, Peter Stormare", "Adventure", "15", 103)
